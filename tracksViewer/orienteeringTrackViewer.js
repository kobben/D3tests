...
var svgMapPanel = d3.select("body")
  ...
  ;
if (background_image != "") {
  var svgBackgroundMap = svgMapPanel.append("image");
  svgBackgroundMap
...
//  .defer(d3.json, dataFile[2])
  .await(drawAllSeries)
;
function drawAllSeries(error, a,b) {
...
// create distline:
  svgTime2Geo.append("line")
    .attr("class","axis")
    .attr("x1",xMargin)
    .attr("y1",(yT - 80) + (series*30))
    .attr("x2",xMargin + (timeScale(latestTime[series])) )
    .attr("y2",(yT - 80) + (series*30))
  ;
  svgTime2Geo.append("circle")
    .attr("class","nostop")
    .attr("cx",xMargin)
    .attr("cy",(yT - 80) + (series*30))
    .attr("r",2)
  ;
  svgTime2Geo.append("circle")
    .attr("class","nostop")
...
//  el.setAttribute("height", el.getAttribute("height")*2);
  ...
  infoStr += "stop_id: " + data.properties.stop_id;
  infoStr += "<hr>";
