...
svgMapPanel.append("text")
  ...
  .text("GEOGRAPHY (click & hold to separate)")
;
//create maps + legend:
...
svgMapPanel
  .on("mousedown", function (d) {
    svgMap[0].transition().ease(d3.easeElasticInOut).duration(1500).attr("transform","translate(-120,-15)")
    svgMap[2].transition().ease(d3.easeElasticInOut).duration(1500).attr("transform","translate(120,15)")
  } )
  .on("mouseup", function (d) {
    svgMap[0].transition().ease(d3.easeElasticInOut).duration(1500).attr("transform","translate(0,0)")
    svgMap[2].transition().ease(d3.easeElasticInOut).duration(1500).attr("transform","translate(0,0)")
  } )
;
...
// *******************
var SVGscaleWidth = 650, SVGscaleHeight = 170; SVGscaleHeightSmall = 130;
...
var svgGeo2Time = new Array(numTimeSeries);
for (i=0; i < numTimeSeries; i++) {
  svgGeo2Time[i] = d3.select("body").append("svg")
    .attr("id", "svgGeo2Time" + i)
    .attr("width", SVGscaleWidth)
    .attr("height", SVGscaleHeightSmall)
  ;
}
...
;
for (i=0; i < numTimeSeries; i++) {
  svgGeo2Time[i].append("text")
    .attr("x", 5)
    .attr("y", 20)
    .attr("class", "panelTitle")
    .text("FROM GEOGRAPHY TO TIME for " + seriesNames[i] )
  ;
}
...
for (i=0; i < numTimeSeries; i++) {
  svgGeo2Time[i].append("g")
    .attr("class", "axis")
    .attr("transform", "translate(" + xMargin + "," + (60 ) + ")")
    .call(distAxisGen[i])
  ;
//  svgGeo2Time[i].append("g")
...
//  ;
  ...
  svgGeo2Time[i].append("text")
    ...
    .attr("fill", "grey")
    .attr("font-size", 10)
    .text("DISTANCE for " + seriesNames[i] )
  ;
}
...
// and draw the non-sliced stuff for Geo2Time panel:
...
//TODO: make proper loop iso this monstrosity...
queue()
...
//            .data(data.filter(function(d){ return d.properties.stop == 1; })) //filter out non-stops
    .enter()
    .append("rect")
    .attr("id", function (d) {return "t2g_d_"  + series +"_" + d.properties.id})
...
// **********************
function drawGeo2TimePanel(data, series) {


//create connectors
  svgGeo2Time[series].selectAll(".connectors")
//    .data(data.filter(function(d){ return d.properties.stop == 1; })) //filter out non-stops
    .data(data)
    .enter()
    .append("line")
...
//        if (data[j].properties.stop == 1) {//filter out non-stops
    var earliestTime = parseMyDate(data[j].properties.departure, series);
    var latestTime = parseMyDate(data[j+1].properties.arrival, series);
    // from end of this stop...
...
//create distance circles
  svgGeo2Time[series].selectAll("distancecircle_"+ series)
    .data(data.filter(function(d){ return d.properties.stop == 1; })) //filter out non-stops
    .enter()
    .append("circle")
    .attr("id", function (d) {return "g2t_t_" + series +"_" + d.properties.id})
    .attr("class", "distancecircle_"+ series)
    .attr("cx", function(d) {return xMargin+(distScale(d.properties.distance))})
    .attr("cy", 90)
    .attr("r", 3)
...
//create time circles
  svgGeo2Time[series].selectAll("distancecircle_"+ series)
    ...
    .enter()
    .append("circle")
    .attr("id", function (d) {return "g2t_d_" + series +"_" + d.properties.id})
    .attr("class", "distancecircle_"+ series)
    .attr("cx", function(d) {return xMargin+(distScale(d.properties.distance))})
    .attr("cy", 60)
    .attr("r", 3)
...
function selectMe(elem, data) {
  var dataID = elem.getAttribute("id").split("_");
  var panel = dataID[0];
  var type = dataID[1];
  var series = dataID[2];
  var id = dataID[3];

  var el = document.getElementById("m_d_" + series + "_" + id);
  el.setAttribute("r", el.getAttribute("r")*3);
//  el = document.getElementById("m_d_" + series + "_" + id);
//  el.setAttribute("r", el.getAttribute("r")*3);
  el = document.getElementById("g2t_d_" + series + "_" + id);
  el.setAttribute("r", el.getAttribute("r")*3);
  el = document.getElementById("g2t_t_" + series + "_" + id);
  el.setAttribute("r", el.getAttribute("r")*3);
//  el = document.getElementById("g2t_t_"  + series + "_" + id);
//  el.setAttribute("r", el.getAttribute("r")*3);
//  el = document.getElementById("nor_t_"  + series + "_" + id);
//  el.setAttribute("height", el.getAttribute("height")*2);
//  el = document.getElementById("t2g_d_" + series + "_" + id);
//  el.setAttribute("height", el.getAttribute("height")*2);
  el = document.getElementById("t2g_d_"  + series + "_" + id);
  el.setAttribute("height", el.getAttribute("height")*2);
...
function unselectMe(elem) {var dataID = elem.getAttribute("id").split("_");
  var panel = dataID[0];
  var type = dataID[1];
  var series = dataID[2];
  var id = dataID[3];
